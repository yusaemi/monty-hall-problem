...
public final class ProblemFlow {
    ...
    private static boolean manuallyStart(String position) throws InterruptedException, IOException {
        ...
        IntStream.range(0, 3).forEach(v -> {
            result.add("pickOn".equals(data.getPools().get(v)) ? "★" : "☆");
        ...
        // System.out.println("╘══╧══╧══╛");
        return "pickOn".equals(data.getPools().get(Integer.parseInt(position) - 1));
    }

    private static void autoStart(int frequency) throws IOException, InterruptedException {
